...
const _restoreContent = (cache, str) => {
  return str.replace(rPlaceholder, (_, index) => {
    assert(cache[index]);
...
class PostRenderCache {
  ...
  loadContent(str) {
    ...
    let finished = false;
    while (!finished) {
      // while loop for nested escaping
      restored = _restoreContent(this.cache, str);
      if (rPlaceholder.test(restored)) finished = true;
    }
