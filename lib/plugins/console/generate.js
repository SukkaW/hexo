...
class Generater {
  ...
  firstGenerate() {
    ...
    return stat(publicDir).then(stats => {
      ...
      const publicFiles = Cache.filter(({ _id }) => {
        let i = 0;
        return _id.charCodeAt(i++) === 112/* p */
            && _id.charCodeAt(i++) === 117/* u */
            && _id.charCodeAt(i++) === 98 /* b */
            && _id.charCodeAt(i++) === 108/* l */
            && _id.charCodeAt(i++) === 105/* i */
            && _id.charCodeAt(i++) === 99 /* c */
            && _id.charCodeAt(i++) === 47;/* / */
