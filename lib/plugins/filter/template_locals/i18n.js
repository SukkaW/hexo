...
const castArray = input => { return Array.isArray(input) ? input : [input]; };
...
function i18nLocalsFilter(locals) {
  ...
  const languages = [...new Set([...castArray(lang), ...castArray(i18nConfigLanguages), ...castArray(i18nLanguages)].filter(Boolean))];
