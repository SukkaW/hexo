...
function fullDateHelper(date, format) {
  ...
  return `${dateHelper.call(this, date)} ${timeHelper.call(this, date)}`;
}
...
function timeTagHelper(date, format) {
  return `<time datetime="${toISOString(date)}">${dateHelper.call(this, date, format, getLanguage(this), this.config.timezone)}</time>`;
}
