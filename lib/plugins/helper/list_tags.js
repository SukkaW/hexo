...
function listTagsHelper(tags, options) {
  ...
  if (style === 'list') {
    ...
    tags.forEach(tag => {
      ...
      result += `<a class="${className}-list-link" href="${url_for.call(this, tag.path)}${suffix}" rel="tag">`;
    ...
    tags.forEach((tag, i) => {
      ...
      result += `<a class="${className}-link" href="${url_for.call(this, tag.path)}${suffix}" rel="tag">`;
      result += transform ? transform(tag.name) : tag.name;
